...
from numba import int32, float32, types, typed, jit
...
class City(object):
    ...
    def plot_grid(self, city_n=100):
        fig, ax = plt.subplots()
        cmap = sns.color_palette(["black", "forestgreen", "gold", "navy", "grey"])
        activity_grid = np.array([obj.value for row in self.grid for obj in row]).reshape(city.n,city.n)
